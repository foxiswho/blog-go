...
// GetMapByNo 多个
func (n *Cache) GetMapByNo(ctx context.Context, list []string) (maps map[string]*entityRam.RamLevelEntity) {
	maps = make(map[string]*entityRam.RamLevelEntity)
	//
	...
	if strPg.IsNotBlank(ret) && ret != "[false]" {
		...
		if nil != valueList && len(valueList) > 0 {
			...
			for _, item := range valueList {
				...
				var vo entityRam.RamLevelEntity
...
// GetNo 单个
func (n *Cache) GetNo(ctx context.Context, no string) (*entityRam.RamLevelEntity, bool) {
	key := n.KeyNo(no)
