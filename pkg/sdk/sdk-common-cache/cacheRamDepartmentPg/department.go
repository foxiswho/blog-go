...
func (n *Cache) KeyNo(no string) string {
	return "cac_department:" + no
}

// GetMapByNo 多个
func (n *Cache) GetMapByNo(ctx context.Context, list []string) (maps map[string]*entityRam.RamDepartmentEntity) {
	maps = make(map[string]*entityRam.RamDepartmentEntity)
	//
	...
	if strPg.IsNotBlank(ret) && ret != "[false]" {
		...
		if nil != valueList && len(valueList) > 0 {
			...
			for _, item := range valueList {
				...
				var vo entityRam.RamDepartmentEntity
...
// GetNo 单个
func (n *Cache) GetNo(ctx context.Context, no string) (*entityRam.RamDepartmentEntity, bool) {
	key := n.KeyNo(no)
