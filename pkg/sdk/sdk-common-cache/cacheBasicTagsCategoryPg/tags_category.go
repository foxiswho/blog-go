package cacheBasicTagsCategoryPg
...
func (n *Cache) KeyNo(no string) string {
	return "cac_basic_tags_category:" + no
}

// GetMapByNo 多个
func (n *Cache) GetMapByNo(ctx context.Context, list []string) (maps map[string]*entityBasic.BasicTagsCategoryEntity) {
	maps = make(map[string]*entityBasic.BasicTagsCategoryEntity)
	//
	...
	if strPg.IsNotBlank(ret) && ret != "[false]" {
		...
		if nil != valueList && len(valueList) > 0 {
			...
			for _, item := range valueList {
				...
				var vo entityBasic.BasicTagsCategoryEntity
...
// GetNo 单个
func (n *Cache) GetNo(ctx context.Context, no string) (*entityBasic.BasicTagsCategoryEntity, bool) {
	key := n.KeyNo(no)
