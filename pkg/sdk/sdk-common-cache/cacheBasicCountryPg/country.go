...
func (n *Cache) KeyNo(no string) string {
	return "cac_basic_country:" + no
}

// GetMapByNo 多个
func (n *Cache) GetMapByNo(ctx context.Context, list []string) (maps map[string]*entityBasic.BasicCountryEntity) {
	maps = make(map[string]*entityBasic.BasicCountryEntity)
	//
	...
	if strPg.IsNotBlank(ret) && ret != "[false]" {
		...
		if nil != valueList && len(valueList) > 0 {
			...
			for _, item := range valueList {
				...
				var vo entityBasic.BasicCountryEntity
...
// GetNo 单个
func (n *Cache) GetNo(ctx context.Context, no string) (*entityBasic.BasicCountryEntity, bool) {
	key := n.KeyNo(no)
