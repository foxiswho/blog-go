...
// GetMapByNo 多个
func (n *Cache) GetMapByNo(ctx context.Context, list []string) (maps map[string]*entityRam.RamTeamEntity) {
	maps = make(map[string]*entityRam.RamTeamEntity)
	//
	...
	if strPg.IsNotBlank(ret) && ret != "[false]" {
		...
		if nil != valueList && len(valueList) > 0 {
			...
			for _, item := range valueList {
				...
				var vo entityRam.RamTeamEntity
...
// GetNo 单个
func (n *Cache) GetNo(ctx context.Context, no string) (*entityRam.RamTeamEntity, bool) {
	key := n.KeyNo(no)
