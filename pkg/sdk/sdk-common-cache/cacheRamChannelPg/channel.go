...
func (n *Cache) KeyNo(no string) string {
	return "cac_channel:" + no
}

// GetMapByNo 多个
func (n *Cache) GetMapByNo(ctx context.Context, list []string) (maps map[string]*entityRam.RamChannelEntity) {
	maps = make(map[string]*entityRam.RamChannelEntity)
	//
	...
	if strPg.IsNotBlank(ret) && ret != "[false]" {
		...
		if nil != valueList && len(valueList) > 0 {
			...
			for _, item := range valueList {
				...
				var vo entityRam.RamChannelEntity
...
// GetNo 单个
func (n *Cache) GetNo(ctx context.Context, no string) (*entityRam.RamChannelEntity, bool) {
	key := n.KeyNo(no)
