package cacheBlogCollectCategoryPg
...
func (n *Cache) KeyNo(no string) string {
	return "cac_blog_collect_category:" + no
}

// GetMapByNo 多个
func (n *Cache) GetMapByNo(ctx context.Context, list []string) (maps map[string]*entityBlog.BlogCollectCategoryEntity) {
	maps = make(map[string]*entityBlog.BlogCollectCategoryEntity)
	//
	...
	if strPg.IsNotBlank(ret) && ret != "[false]" {
		...
		if nil != valueList && len(valueList) > 0 {
			...
			for _, item := range valueList {
				...
				var vo entityBlog.BlogCollectCategoryEntity
...
// GetNo 单个
func (n *Cache) GetNo(ctx context.Context, no string) (*entityBlog.BlogCollectCategoryEntity, bool) {
	key := n.KeyNo(no)
