...
func init() {
	//登陆
	gs.Provide(&controller.LoginController{}).Init(func(s *controller.LoginController) {
	...
	//退出
	gs.Provide(&controller.LogoutController{}).Init(func(s *controller.LogoutController) {
		r := ginServer.GinServerDefault
		group := r.Group("/pg2lq/auth/sys")
