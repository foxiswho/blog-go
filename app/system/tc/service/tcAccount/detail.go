...
func (c *Detail) Process(ctx *gin.Context, id string) (rt rg.Rs[modRamAccount.DetailVo]) {
	...
	find, b := c.sp.accDb.FindByIdAndTypeDomain(ctx, numberPg.StrToInt64(id), c.tp.ToTypeDomain().String())
	if !b {
	...
	//租户
	if nil != find.Os.Data().Tenants && len(find.Os.Data().Tenants) > 0 {
		for _, obj := range find.Os.Data().Tenants {
			idsTenant = append(idsTenant, obj)
		}
	}
	if strPg.IsNotBlank(find.TenantNo) {
		idsTenant = append(idsTenant, find.TenantNo)
	}
	//部门
	{
		if len(idsDep) > 0 {
	...
	{
		if len(idsTenant) > 0 {
			infos, result := c.sp.tenDb.FindAllByNoIn(ctx, idsTenant)
			if result {
	...
	//租户
	if nil != find.Os.Data().Tenants && len(find.Os.Data().Tenants) > 0 {
		info.Os.No.Tenants = find.Os.Data().Tenants
		info.Os.NoName.Tenants = make([]string, 0)
		for _, obj := range find.Os.Data().Tenants {
			if get, ok := mapTenant[obj]; ok {
				info.Os.NoName.Tenants = append(info.Os.NoName.Tenants, get.Name)
			}
		}
	}
	if strPg.IsNotBlank(info.TenantNo) {
		if get, ok := mapTenant[info.TenantNo]; ok {
			info.TenantNoName = get.Name
		}
	}
