...
func (c *RamDepartmentService) Update(ctx *gin.Context, ct modRamDepartment.UpdateCt) (rt rg.Rs[string]) {
	...
	if strPg.IsNotBlank(ct.ParentNo) {
		...
		if parent.No != find.ParentNo {
			result2 := false
			childData, result2 = r.FindAllByNoLink(find.IdLink, repositoryPg.WithCtxOption(ctx))
			if result2 {
				//c.log.Infof("data=%+v \n", childData)
...
func (c *RamDepartmentService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	...
	ctxR := c.sv
	finds, b := ctxR.FindAllByIdStringIn(ids, repositoryPg.WithCtxOption(ctx))
	if !b {
		return rt.ErrorMessage("数据不存在")
	}
...
func (c *RamDepartmentService) Query(ctx *gin.Context, ct modRamDepartment.QueryCt) (rt rg.Rs[pagePg.Paginator[modRamDepartment.Vo]]) {
	...
	rt.Data.Data = slice
	r := c.sv
	page, err := r.FindAllPage(ctx, query, func(p *pagePg.PageCondition[*entityRam.RamDepartmentEntity]) {
