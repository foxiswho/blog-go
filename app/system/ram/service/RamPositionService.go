...
import (
	...
	"github.com/foxiswho/blog-go/app/system/ram/model/modRamPosition"
...
func init() {
	gs.Provide(new(RamPositionService)).Init(func(s *RamPositionService) {
...
func (c *RamPositionService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	...
	for _, info := range finds {
		if info.State != state.IndexInt8() {
			r.Update(entityRam.RamPositionEntity{State: state.IndexInt8()}, info.ID)
		}
	}
	return rt.Ok()
}
	...
	if c.sv.Config().Data.Delete {
		...
		for _, info := range finds {
			...
			if ok, reverse := enum.ReverseEnableDisable(); ok {
				repository.Update(entityRam.RamPositionEntity{State: reverse.IndexInt8()}, info.ID)
			}
		}
	}
	return rt.Ok()
...
//	@receiver c
//	@param ct
func (c *RamPositionService) Query(ctx *gin.Context, ct modRamPosition.QueryCt) (rt rg.Rs[pagePg.PaginatorPg[modRamPosition.Vo]]) {
	c.log.Infof("ct=%+v", ct)
	...
	rt.Data.Data = slice
	r := c.sv
	page, err := r.FindAllPageQuery(query, func(p *pagePg.PageCondition[*entityRam.RamPositionEntity]) {
		p.PageOption = func(c *pagePg.PaginatorPg[*entityRam.RamPositionEntity]) {
	...
	if page.Total > 0 && page.Data != nil && len(page.Data) > 0 {
		...
		pg := pagePg.NewPaginatorPg(func(c *pagePg.PaginatorPg[modRamPosition.Vo]) {
