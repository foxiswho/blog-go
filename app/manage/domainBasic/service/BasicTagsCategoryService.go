...
import (
	...
	"github.com/foxiswho/blog-go/app/manage/domainBasic/model/modBasicTagsCategory"
...
func init() {
	gs.Provide(new(BasicTagsCategoryService)).Init(func(s *BasicTagsCategoryService) {
...
func (c *BasicTagsCategoryService) Create(ctx *gin.Context, ct modBasicTagsCategory.CreateCt) (rt rg.Rs[string]) {
	...
	if strPg.IsNotBlank(info.ParentNo) {
		parent, result = r.FindByNo(info.ParentNo)
		if !result {
	...
	if strPg.IsNotBlank(ct.ParentNo) && nil != childData {
		...
		for _, val := range maps {
			for _, item := range val {
				...
				r.Update(entityBasic.BasicTagsCategoryEntity{IdLink: item.IdLink,
...
func (c *BasicTagsCategoryService) CacheOverride(ctx *gin.Context) {
	...
	for _, val := range maps {
		for _, item := range val {
			r.Update(entityBasic.BasicTagsCategoryEntity{
				IdLink: item.IdLink,
				NoLink: item.NoLink},
				item.ID)
		}
...
func (c *BasicTagsCategoryService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	...
	for _, info := range finds {
		if info.State != state.IndexInt8() {
			ctxR.Update(entityBasic.BasicTagsCategoryEntity{State: state.IndexInt8()}, info.ID)
		}
	}
	return rt.Ok()
}
	...
	if c.sv.Config().Data.Delete {
		...
		for _, info := range finds {
			...
			if ok, reverse := enum.ReverseEnableDisable(); ok {
				repository.Update(entityBasic.BasicTagsCategoryEntity{State: reverse.IndexInt8()}, info.ID)
			}
		}
	}
...
//	@receiver c
//	@param ct
func (c *BasicTagsCategoryService) QueryPublic(ctx *gin.Context, ct modBlogCategory.QueryCt) (rt rg.Rs[pagePg.PaginatorPg[modBlogCategory.Vo]]) {
	var query entityBasic.BasicTagsCategoryEntity
	copier.Copy(&query, &ct)
