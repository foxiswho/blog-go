...
type Vo struct {
	...
	StateBind     typePg.Int8         `json:"stateBind" label:"绑定状态:1未绑定;2已绑定" ` // 绑定状态
	...
	BindAno       string              `json:"bindAno" label:"绑定账号" `        // 绑定账号
	ExternalSub   string              `json:"externalSub" label:"外部身份标识" ` // 外部身份标识
	BindTime      *time.Time          `json:"bindTime" label:"绑定时间" `       // 绑定时间
	UnBindTime    *time.Time          `json:"unBindTime" label:"解绑时间" `     // 解绑时间
	LastLoginTime *time.Time          `json:"lastLoginTime" label:"最后登录时间" ` // 最后登录时间
