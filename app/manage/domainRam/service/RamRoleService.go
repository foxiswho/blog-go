...
import (
	...
	"github.com/foxiswho/blog-go/app/manage/domainRam/model/modRamRole"
...
func init() {
	gs.Provide(new(RamRoleService)).Init(func(s *RamRoleService) {
...
func (c *RamRoleService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	...
	for _, info := range finds {
		if info.State != state.IndexInt8() {
			r.Update(entityRam.RamRoleEntity{State: state.IndexInt8()}, info.ID)
		}
	}
	return rt.Ok()
}
	...
	if c.sv.Config().Data.Delete {
		...
		for _, info := range finds {
			...
			if ok, reverse := enum.ReverseEnableDisable(); ok {
				repository.Update(entityRam.RamRoleEntity{State: reverse.IndexInt8()}, info.ID)
			}
		}
	}
	return rt.Ok()
...
//	@receiver c
//	@param ct
func (c *RamRoleService) Query(ctx *gin.Context, ct modRamRole.QueryCt) (rt rg.Rs[pagePg.PaginatorPg[modRamRole.Vo]]) {
	c.log.Infof("ct=%+v", ct)
	...
	rt.Data.Data = slice
	r := c.sv
	page, err := r.FindAllPageQuery(query, func(p *pagePg.PageCondition[*entityRam.RamRoleEntity]) {
		p.PageOption = func(c *pagePg.PaginatorPg[*entityRam.RamRoleEntity]) {
	...
	if page.Total > 0 && page.Data != nil && len(page.Data) > 0 {
		...
		pg := pagePg.NewPaginatorPg(func(c *pagePg.PaginatorPg[modRamRole.Vo]) {
