...
import (
	...
	"github.com/foxiswho/blog-go/app/manage/domainRam/model/modRamApp"
...
func init() {
	gs.Provide(new(RamAppService)).Init(func(s *RamAppService) {
...
//	@param ct
//	@return rt
func (c *RamAppService) Create(ctx *gin.Context, ct modRamApp.CreateCt) (rt rg.Rs[string]) {
	c.log.Infof("ct=%+v", ct)
...
//	@param ct
//	@return rt
func (c *RamAppService) Update(ctx *gin.Context, ct modRamApp.UpdateCt) (rt rg.Rs[string]) {
	c.log.Infof("ct=%#v", ct)
	var info entityRam.RamAppEntity
	copier.Copy(&info, &ct)
	r := c.sv
...
//	@receiver c
//	@param id
func (c *RamAppService) Detail(ctx *gin.Context, id int64) (rt rg.Rs[modRamApp.Vo]) {
	if id < 1 {
		return rt.ErrorMessage("id错误")
	}
...
func (c *RamAppService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	...
	for _, info := range finds {
		if info.State != state.IndexInt8() {
			r.Update(entityRam.RamAppEntity{State: state.IndexInt8()}, info.ID)
		}
	}
	return rt.Ok()
}
	...
	if c.sv.Config().Data.Delete {
		...
		for _, info := range finds {
			...
			if ok, reverse := enum.ReverseEnableDisable(); ok {
				repository.Update(entityRam.RamAppEntity{State: reverse.IndexInt8()}, info.ID)
			}
		}
	}
	return rt.Ok()
...
//	@receiver c
//	@param ct
func (c *RamAppService) Query(ctx *gin.Context, ct modRamApp.QueryCt) (rt rg.Rs[pagePg.PaginatorPg[modRamApp.Vo]]) {
	c.log.Infof("ct=%+v", ct)
	...
	rt.Data.Data = slice
	r := c.sv
	page, err := r.FindAllPageQuery(query, func(p *pagePg.PageCondition[*entityRam.RamAppEntity]) {
		p.PageOption = func(c *pagePg.PaginatorPg[*entityRam.RamAppEntity]) {
	...
	if page.Total > 0 && page.Data != nil && len(page.Data) > 0 {
		...
		pg := pagePg.NewPaginatorPg(func(c *pagePg.PaginatorPg[modRamApp.Vo]) {
...
//	@receiver c
//	@param ct
func (c *RamAppService) SelectNodePublic(ctx *gin.Context, ct modRamApp.QueryPublicCt) (rt rg.Rs[[]model.BaseNodeNo]) {
	c.log.Infof("ct=%+v", ct)
	var query entityRam.RamAppEntity
	copier.Copy(&query, &ct)
...
//	@receiver c
//	@param ct
func (c *RamAppService) SelectNodeAllPublic(ctx *gin.Context, ct modRamApp.QueryPublicCt) (rt rg.Rs[[]model.BaseNodeNo]) {
	c.log.Infof("ct=%+v", ct)
	var query entityRam.RamAppEntity
	copier.Copy(&query, &ct)
	slice := make([]model.BaseNodeNo, 0)
...
//	@receiver c
//	@param ct
func (c *RamAppService) SelectPublic(ctx *gin.Context, ct modRamApp.QueryCt) (rt rg.Rs[[]modRamApp.Vo]) {
	c.log.Infof("ct=%+v", ct)
	var query entityRam.RamAppEntity
	copier.Copy(&query, &ct)
	slice := make([]modRamApp.Vo, 0)
