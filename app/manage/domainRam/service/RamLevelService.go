...
import (
	...
	"github.com/foxiswho/blog-go/app/manage/domainRam/model/modRamLevel"
...
func init() {
	gs.Provide(new(RamLevelService)).Init(func(s *RamLevelService) {
...
func (c *RamLevelService) State(ctx *gin.Context, ids []string, state enumStatePg.State) (rt rg.Rs[string]) {
	...
	for _, info := range finds {
		if info.State != state.IndexInt8() {
			r.Update(entityRam.RamLevelEntity{State: state.IndexInt8()}, info.ID)
		}
	}
	return rt.Ok()
}
	...
	if c.sv.Config().Data.Delete {
		...
		for _, info := range finds {
			...
			if ok, reverse := enum.ReverseEnableDisable(); ok {
				repository.Update(entityRam.RamLevelEntity{State: reverse.IndexInt8()}, info.ID)
			}
		}
	}
	return rt.Ok()
...
//	@receiver c
//	@param ct
func (c *RamLevelService) Query(ctx *gin.Context, ct modRamLevel.QueryCt) (rt rg.Rs[pagePg.PaginatorPg[modRamLevel.Vo]]) {
	c.log.Infof("ct=%+v", ct)
	...
	rt.Data.Data = slice
	r := c.sv
	page, err := r.FindAllPageQuery(query, func(p *pagePg.PageCondition[*entityRam.RamLevelEntity]) {
		p.PageOption = func(c *pagePg.PaginatorPg[*entityRam.RamLevelEntity]) {
	...
	if page.Total > 0 && page.Data != nil && len(page.Data) > 0 {
		...
		pg := pagePg.NewPaginatorPg(func(c *pagePg.PaginatorPg[modRamLevel.Vo]) {
