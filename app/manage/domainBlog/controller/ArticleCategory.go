...
func (c *ArticleCategoryController) RegisterRoutes(e *gin.Engine) {
	group := e.Group("/pg2lq/manage/blog/article-category", authPg.GroupManageMiddleware(c.Sp))
...
func (c *ArticleCategoryController) SelectNodePublic(ctx *gin.Context) {
...
func (c *ArticleCategoryController) SelectPublic(ctx *gin.Context) {
	var ct modBlogArticleCategory.QueryPublicCt
	if err := ctx.ShouldBind(&ct); err != nil {
		//对 返回 错误进行转义 成中文
		translate := validatorPg.Translate(err, &ct)
