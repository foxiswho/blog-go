...
// TemplateController oprations for Template
...
// @Success 201 {int} models.Template
// @Failure 403 body is empty
// @router / [post]
func (c *TemplateController) Post() {
	var v models.Template
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if _, err := models.AddTemplate(&v); err == nil {
...
func (c *TemplateController) GetOne() {
	...
	v, err := models.GetTemplateById(id)
	if err != nil {
		c.Data["json"] = err.Error()
	} else {
		c.Data["json"] = v
...
func (c *TemplateController) GetAll() {
	...
	l, err := models.GetAllTemplate(query, fields, sortby, order, offset, limit)
	if err != nil {
		c.Data["json"] = err.Error()
	} else {
		c.Data["json"] = l
...
func (c *TemplateController) Put() {
	idStr := c.Ctx.Input.Param(":id")
	id, _ := strconv.Atoi(idStr)
	v := models.Template{Id: id}
	if err := json.Unmarshal(c.Ctx.Input.RequestBody, &v); err == nil {
		if err := models.UpdateTemplateById(&v); err == nil {
			c.Data["json"] = "OK"
		} else {
			c.Data["json"] = err.Error()
		}
