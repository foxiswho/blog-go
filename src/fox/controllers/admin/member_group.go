...
func (c *MemberGroup)List() {
	...
	data, err := ser.Query(member.MEMBER_GROUP)
	fmt.Println(err)
	c.Data["data"] = data
	c.Data["title"] = "用户组-列表"
...
func (c *MemberGroup)Add() {
	mod := model.NewType()
	mod.TypeId = member.MEMBER_GROUP
	mod.IsDefault = 0
	mod.IsDel = 0
	mod.IsSystem = 0
	mod.IsShow = 1
	mod.IsChild = 0
	c.Data["type_id"] = member.MEMBER_GROUP
...
func (c *MemberGroup)CheckName() {
	...
	//创建
	ser := admin.NewTypeService()
	ok, err := ser.CheckNameTypeId(member.MEMBER_GROUP, name, id)
	if err != nil {
		rsp.Error(err.Error())
	} else {
		fmt.Println("成功！:", ok)
...
func (c *MemberGroup)Delete() {
	...
	//更新
	ser := admin.NewTypeService()
	_, err := ser.DeleteAndTypeId(int_id,member.MEMBER_GROUP)
	if err != nil {
		rsp.Error(err.Error())
	} else {
		rsp.Success("")
