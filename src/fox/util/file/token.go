...
func TokeMake(maps map[string]interface{}) (string, error) {
	...
	if err != nil {
		return "", &util.Error{Msg:"加密失败：" + err.Error()}
	}
...
func TokenDeCode(str string) (map[string]interface{}, error) {
	if len(str) < 1 {
		return nil, &util.Error{Msg:"字符串 不能为空"}
	}
	key := []byte(beego.AppConfig.String("aes_key"))
	b64:=base64.NewEncoding("ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/")
	byt, err := b64.DecodeString(str)
	if err != nil {
		return nil, &util.Error{Msg:"base64解码失败：" + err.Error()}
	}
	origData, err := crypt.AesDecrypt(byt, key)
	if err != nil {
		return nil, &util.Error{Msg:"解密失败：" + err.Error()}
	}
	maps, err := array.StrToMap(string(origData))
	if err != nil {
		return nil, &util.Error{Msg:"转换为map失败：" + err.Error()}
	}
	return maps, nil
}