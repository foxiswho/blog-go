...
type AdminRoleAccess struct {
	Aid    int `json:"aid" xorm:"default 0 unique(aid_role_id) INT(11)" orm:"column(aid)"`
	RoleId int `json:"role_id" xorm:"default 0 unique(aid_role_id) INT(11)" orm:"column(role_id)"`
}
...
//列表查询
func (c *AdminRoleAccess) GetAll(q map[string]interface{}, fields []string, orderBy string, page int, limit int) (*db.Page, error) {
	session := db.Filter(q)
